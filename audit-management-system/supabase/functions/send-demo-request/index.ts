...
const toAddress = Deno.env.get("DEMO_REQUEST_TO");
const fromAddress = Deno.env.get("DEMO_REQUEST_FROM") ?? "CleanAudits <demo@yourdomain.com>";
...
serve(async (req) => {
  ...
  try {
                ...
                <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#FFFFFF;border:1px solid #E0E3E7;border-radius:16px;overflow:hidden;box-shadow:0 12px 28px rgba(12,17,22,0.08);">
                  <tr>
                    <td style="padding:20px 22px;border-bottom:1px solid #E0E3E7;background:linear-gradient(135deg,#FFFFFF 0%,#F3F5F8 50%,#FFFFFF 100%);">
                      <p style="margin:0;font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#546E7A;">Demo Intake</p>
                  ...
                  <tr>
                    <td style="padding:22px 24px;color:#1B1B1B;">
                      <p style="margin:0 0 14px;font-size:15px;letter-spacing:0.1px;color:#1B1B1B;">
                        <span>A new demo was requested through the </span>
    ...
    // Optional: send a confirmation to the requester
    await resend.emails.send({
      from: fromAddress,
      to: [email],
                  ...
                  <tr>
                    <td style="padding:22px 24px;color:#1B1B1B;">
                      ...
                      <p style="margin:0 0 18px;font-size:15px;line-height:1.6;">
                        <span>Thanks for requesting a </span>
                        <span style="color:#C8A951;font-weight:900;">Clean</span><span style="color:#1C2A3A;font-weight:900;">Audits</span>
                        <span> demo. Our team will reach out soon to schedule your session.</span>
                      </p>
                      <p style="margin:0 0 18px;font-size:15px;line-height:1.6;">
                        <span>Please keep an eye on your inbox—another email with your magic link and a quick video from our team is on its way.</span>
                      </p>
